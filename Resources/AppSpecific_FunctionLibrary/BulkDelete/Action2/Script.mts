...
DeleteOpportunitiesFromOtherOwners=DataTable.Value("DeleteOpportunitiesFromOtherOwners","BulkDelete")
	...
	If RunTest = "Yes" Then
		...
		Wait 5 
		If Browser("Home | Salesforce").Page("Home | Salesforce").WebButton("Setup").Exist(conExistTimeout) Then
			'Click on Setting icon
			Browser("Home | Salesforce").Page("Home | Salesforce").WebButton("Setup").Click
			Wait 3
			'Click on Setup link
			Browser("Home | Salesforce").Page("Home | Salesforce").Link("Setup").Click
			'Wait for Setup home page to load
			Browser("Home | Salesforce").Page("Setup Home | Salesforce").Sync
			'Check if Setup Webbutton is present or not
			If Browser("Home | Salesforce").Page("Setup Home | Salesforce").WebButton("Setup").Exist(conExistTimeout) Then
				'Enter the search key in the quick find box
				Browser("Home | Salesforce").Page("Setup Home | Salesforce").WebEdit("Quick Find").Set "Mass Delete Records"
				Browser("Setup Home | Salesforce").Page("Setup Home | Salesforce").WebEdit("Quick Find").Click
				Wait 3
				'Click on Mass Delete Records link
				Browser("Home | Salesforce").Page("Setup Home | Salesforce").Link("Mass Delete Records").Click
				Wait 
				'Click on the search result
				Browser("Home | Salesforce").Page("Setup Home | Salesforce").Frame("Frame1").Link("Mass Delete Accounts Search Link").Click
				Wait 5 
				'Check if the FilterTable is loaded properly
				If Browser("Mass Delete Records Page").Page("Mass Delete Records").Frame("vfFrameId_1499840695711").WebTable("FilterTable").Exist(conExistTimeout) Then
					'Find how many filters are provided in the datatable
					For Iterator = 1 To 5 Step 1
						If IsNull(Filt(Iterator)) Then
